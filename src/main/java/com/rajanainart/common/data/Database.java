...
public final class Database implements Closeable {
    ...
    public static void bindPositionalParameters(Query query, Parameter[] parameters) {
        int index = 1;
        for (Parameter p : parameters)
            query.setParameter(index++, p.getValue(), p.getType());
    }

    public static void bindParameters(Query query, Parameter[] parameters) {
        for (Parameter p : parameters)

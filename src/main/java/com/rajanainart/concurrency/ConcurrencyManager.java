            ...
            for (BaseConcurrencyThread t : threads) {
                if (t.getIsComplete()) {
                    ...
                    if (completed.size() % completedCallback == 0 && completed.size() > lastCompleted) {
                        String message = String.format("%s: %s threads out of %s total threads [pool] completed, time elapsed in seconds %s",
                                                        getOperationName(), completed.size(), threads.size(), (System.currentTimeMillis() - startTime)/1000);

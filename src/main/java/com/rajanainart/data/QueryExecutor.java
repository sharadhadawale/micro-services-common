...
public class QueryExecutor implements Closeable {
    ...
    public Database.Parameter[] buildDbParameters() {
        ...
        for (RestQueryConfig.ParameterConfig p : config.getParameters()) {
            if (!p.isOutput() && p.getType() == RestQueryConfig.ParameterType.SCALAR) {
                if (request.getParams().containsKey(p.getId()))
                    parameters[index++] = db.new Parameter(p.getId(), request.getParams().get(p.getId()));
                else
                    parameters[index++] = db.new Parameter(p.getId(), p.getDefaultValue());
            }
            else if (!p.isOutput() && p.getType() == RestQueryConfig.ParameterType.STRING_ARRAY) {
                if (request.getParamsWithObject().containsKey(p.getId()))
                    parameters[index++] = db.new Parameter(p.getId(), p.getName(), StringType.INSTANCE, request.getParamsWithObject().get(p.getId()), p.isOutput(), p.getType());
                else
                    parameters[index++] = db.new Parameter(p.getId(), p.getName(), StringType.INSTANCE, p.getDefaultValueAsStringArray(), p.isOutput(), p.getType());
            }
            else if (!p.isOutput() && p.getType() == RestQueryConfig.ParameterType.NUMERIC_ARRAY) {
                if (request.getParamsWithObject().containsKey(p.getId()))
                    parameters[index++] = db.new Parameter(p.getId(), p.getName(), DoubleType.INSTANCE, request.getParamsWithObject().get(p.getId()), p.isOutput(), p.getType());
                else
                    parameters[index++] = db.new Parameter(p.getId(), p.getName(), DoubleType.INSTANCE, p.getDefaultValueAsNumberArray(Long.class), p.isOutput(), p.getType());
            }
            else if (p.isOutput() && p.getType() == RestQueryConfig.ParameterType.CURSOR)
                    parameters[index++] = db.new Parameter(p.getId(), p.getName(), StringType.INSTANCE, null, p.isOutput(), p.getType());
            else if (p.isOutput() && p.getType() == RestQueryConfig.ParameterType.SCALAR)
                parameters[index++] = db.new Parameter(p.getId(), p.getName(), StringType.INSTANCE, null, p.isOutput(), p.getType());
        }
        return parameters;
    }
    ...
    public List<Map<String, Object>> selectDbSpecificMapList() {
        Database.Parameter[] parameters = buildDbParameters();
        if (config.getRestQueryType() == RestQueryConfig.RestQueryType.SELECT &&
            request.getCurrentPageNumber() != -1)
            return db.selectDbSpecificMapList(getPagedQuery(), config.getRestQueryType(), parameters);
        else if (config.getRestQueryType() == RestQueryConfig.RestQueryType.SELECT &&
                 request.hasCurrentPageNumber() && request.getCurrentPageNumber() == -1)
            return db.selectDbSpecificMapList(getFilteredQuery(), config.getRestQueryType(), parameters);
        return db.selectDbSpecificMapList(config.getQuery(), config.getRestQueryType(), parameters);
    }

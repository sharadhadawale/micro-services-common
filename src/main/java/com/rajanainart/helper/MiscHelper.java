...
public class MiscHelper {
    ...
    public static Date truncateTime(Date date) {
        try {
            ...
            String           timeRemoved = format.format(date);
            return format.parse(timeRemoved);
        }
    ...
    public static Map<String, MultipartFile> getUploadedFiles(HttpServletRequest httpServletRequest) {
        ...
        if (httpServletRequest instanceof MultipartHttpServletRequest) {
            MultipartHttpServletRequest mServletRequest = (MultipartHttpServletRequest)httpServletRequest;
            uploadedFiles = mServletRequest.getFileMap();
        }
        return uploadedFiles;
    }

    public static <T extends Enum> T convertStringToEnum(Class<T> clazz, String value, T defaultValue) {
        try {
            return (T)Enum.valueOf(clazz, value.toUpperCase(Locale.ENGLISH));
        }
        catch (Exception ex) {
            return defaultValue;
        }
    ...
    public static String convertRegexCharsToText(String text) {
        return text.replace("$", "\\$")
                   .replace("^", "\\^");
    }
}

...
public class JpaRestController extends BaseRestController {
    ...
    @RequestMapping(value = "/save/getId/{entity:[a-zA-Z0-9]*}", method = RequestMethod.POST)
    @ResponseBody
    public ResponseEntity<BaseEntity> saveAndGetId(@PathVariable("entity") String entityName, @RequestBody Map<String, Object> record) {
        String      escaped = HtmlUtils.htmlEscape(entityName);
        HttpHeaders headers = buildHttpHeaders(RestQueryConfig.RestQueryContentType.JSON.toString());
        Class<?> clazz = AppContext.getClassTypeOf(escaped);
        int id ;
        ...
        if (instance != null) {
            ...
            return new ResponseEntity<>(RestMessageEntity.getInstance("", "Record saved Successfully",RestMessageEntity.MessageStatus.SUCCESS,id), headers, HttpStatus.OK);
        }
        return new ResponseEntity<>(RestMessageEntity.getInstance("","Could not build hibernate entity from the parameters", RestMessageEntity.MessageStatus.FAILURE),headers, HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    @RequestMapping(value = "/delete-all/{entity:[a-zA-Z0-9]*}", method = RequestMethod.POST)
    @ResponseBody
    public ResponseEntity<BaseEntity> deleteAll(@PathVariable("entity") String entityName, @RequestBody Map<String, Object> conditions) {
        String      escaped = HtmlUtils.htmlEscape(entityName);
        HttpHeaders headers = buildHttpHeaders(RestQueryConfig.RestQueryContentType.JSON.toString());
        Class<?>    clazz   = AppContext.getClassTypeOf(escaped);
        if (clazz == null) {
        ...
        return new ResponseEntity<>(RestMessageEntity.getInstance("", "Records deleted Successfully", RestMessageEntity.MessageStatus.SUCCESS), headers, HttpStatus.OK);
    }
}

...
import com.rajanainart.upload.task.UploadTask;
import com.rajanainart.upload.validator.DataValidator;

public class Upload {
    public final static String SUCCESS = "OK";

    public final static String DEFAULT_UPLOAD_TASK = "default-upload-task";
    ...
    public static void downloadFormat(String uploadName, OutputStream stream) throws IOException {
        UploadConfig config = UPLOAD_CONFIGS.get(uploadName);
        ...
        task.onDownloadFormat(config, addl);
        ExcelDocument document = new ExcelDocument(stream, UPLOAD_CONFIGS.get(uploadName).getName()+".xlsx");
        document.writeHeader(UPLOAD_CONFIGS.get(uploadName), addl);
        document.close();
    }
    ...
    public boolean validate() {
        ...
        for (Map<String, Object> record : records) {
            if (current++ == 0) {
                context.setHeader(record);
                context.setCurrentIndex(current);
            }else {
            context.setCurrentIndex(current);
            context.getUploadRecords().clear();
            context.getUploadRecords().add(record);
            index++;
